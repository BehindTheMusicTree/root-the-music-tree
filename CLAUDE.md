...
A `uv` workspace monorepo of data pipelines feeding the BTMT (Behind The Music Tree) ecosystem. Each pipeline is an independent workspace member under `pipelines/`, sharing only the `common` package. All pipelines implement a **Bronze layer** (raw extraction to Parquet); a **Silver layer** (cleaned, joined, hierarchy-built data) is underway for both pipelines: `wikidata` (eight steps, `1_item_links` → `8_canonical_roots` — see below), and `musicbrainz` (three steps so far, `1_recording_link` → `3_song_example` — see below). Gold layer is planned but not yet built.
...
- **Run wikidata Silver:** `uv run --package wikidata python -m wikidata.silver` (reads `BRONZE_OUTPUT_DIR/wikidata_genre_tree.parquet`, runs all eight steps in sequence, writes `SILVER_OUTPUT_DIR/1_item_links.parquet`, `2_non_genre_pruning.parquet`, `3_regional_overview_classification.parquet`, `4_regional_classification.parquet`, `5_canonical_parents.parquet`, `6_canonical_hierarchy.parquet`, `7_regional_hierarchy.parquet`, and `8_canonical_roots.parquet`)
...
**Silver layer, `wikidata` (`pipelines/wikidata/src/wikidata/silver/`):** eight sequential steps, each reading the previous step's output. `1_item_links` derives a browsable Wikidata page URL (`item_url`/`parent_url`) from each row's `item_id`/`parent_id`, and adds `has_item_label`/`has_parent_label` flags, `False` where the label service fell back to printing the QID (no label in any queried language). `2_non_genre_pruning` drops theme/technique/out-of-scope items entirely, via three git-tracked, hand-curated manual CSV backstops (`manual_theme_genres.csv`, `manual_technique_genres.csv`, `manual_out_of_scope_genres.csv`) — this isn't about region, it's non-genre pruning that runs as early as possible, before regional classification (or any other downstream step) sees the tree. `3_regional_overview_classification` flags (not drops) rows whose `item_label` is a Wikidata "music of \<place\>" regional-overview article (e.g. "music of Kenya") rather than an actual genre — it also reads a git-tracked, hand-curated `manual_regional_overview_additions.csv` as a backstop for real overview items entirely absent from Bronze (not even as a `parent_label`). `4_regional_classification` cascades the regional-overview seed set down through parent edges to flag every nationally/ethnically-specific genre (e.g. "fado", "morna") as regional, as opposed to canonical genres like "rock music" — it also reads a git-tracked, hand-curated `manual_regional_overrides.csv` (`regional_reason = "manual_override"`) as a backstop for the rare genre none of the automated seed/`indigenous_to`/`country_of_origin` sources catch. `5_canonical_parents` flags whether each row's parent is itself a real genre. `6_canonical_hierarchy` is the first step that prunes rather than flags: it collapses the edge list to one row per non-regional genre item, producing `6_canonical_hierarchy.parquet` (canonical, excludes regional items). `7_regional_hierarchy` mirrors that pruning restricted to regional items, producing `7_regional_hierarchy.parquet`. `8_canonical_roots` extracts `6_canonical_hierarchy`'s root items (no parent) into their own `8_canonical_roots.parquet`, for manual exploration of the "too many roots" open question below. See `pipelines/wikidata/DESIGN.md` for the rationale and rules behind each step (e.g. the regional cascade's seed set, the multi-parent collapse heuristic — several explicitly marked provisional/exploration-phase there) and `pipelines/wikidata/SCHEMA.md#3-silver` for the full column definitions and profiling detail. `6_canonical_hierarchy` (canonical) currently surfaces a high number of root items; whether that reflects the source data or an upstream pruning artifact is an open exploration — see `pipelines/wikidata/notebooks/explore_genre_tree.ipynb`.

**Target shape (design intent, not yet reached):** the canonical tree (`6_canonical_hierarchy`) should collapse down to a handful of root genre families — rock, blues, jazz, funk/disco, electronic, hip-hop, reggae/dub, classical music, etc. — not the hundreds of roots it currently produces. Getting there is expected to be mostly a linking/cleaning problem (correcting mis-collapsed parent edges, e.g. the multi-parent lowest-QID heuristic), not a new extraction or classification mechanism. The regional tree (`7_regional_hierarchy`) follows different logic entirely: one root per cultural/geographic region (e.g. "music of Cape Verde"), with each region's own genres nested underneath — it is not expected to collapse to a small root count the way the canonical tree is.
