...
A `uv` workspace monorepo of data pipelines feeding the BTMT (Behind The Music Tree) ecosystem. Each pipeline is an independent workspace member under `pipelines/`, sharing only the `common` package. All pipelines implement a **Bronze layer** (raw extraction to Parquet); a **Silver layer** (cleaned, joined, hierarchy-built data) is underway for both pipelines: `wikidata` (seven steps, `1_item_links` → `7_canonical_roots` — see below), and `musicbrainz` (three steps so far, `1_recording_link` → `3_song_example` — see below). Gold layer is planned but not yet built.
...
- **Run wikidata Silver:** `uv run --package wikidata python -m wikidata.silver` (reads `BRONZE_OUTPUT_DIR/wikidata_genre_tree.parquet`, runs all seven steps in sequence, writes `SILVER_OUTPUT_DIR/1_item_links.parquet`, `2_non_genre_pruning.parquet`, `3_regional_overview_classification.parquet`, `4_regional_classification.parquet`, `5_canonical_parents.parquet`, `6_canonical_hierarchy.parquet`, `6_regional_hierarchy.parquet`, and `7_canonical_roots.parquet`)
