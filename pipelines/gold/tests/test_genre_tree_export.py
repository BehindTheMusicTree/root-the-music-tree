...
from gold.genre_tree_export import build_genre_tree, export_genre_tree
...
def test_export_genre_tree_writes_tree_shape(tmp_path: Path) -> None:
    ...
    result = export_genre_tree(wikidata_silver_dir, output_dir)
...
def test_export_genre_tree_creates_output_dir(tmp_path: Path) -> None:
    wikidata_silver_dir = tmp_path / "wikidata_silver"
    wikidata_silver_dir.mkdir()
    _hierarchy().write_parquet(wikidata_silver_dir / "7_canonical_hierarchy.parquet")
    output_dir = tmp_path / "does" / "not" / "exist"

    export_genre_tree(wikidata_silver_dir, output_dir)

    assert output_dir.is_dir()


def test_export_genre_tree_raises_on_schema_violation(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import gold.genre_tree_export as module
    ...
    with pytest.raises(ValueError, match="schema validation"):
        module.export_genre_tree(wikidata_silver_dir, tmp_path / "gold")
