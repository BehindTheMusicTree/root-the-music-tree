...
def prune_regional_hierarchy(canonical_parents_path: Path, output_dir: Path) -> Path:
    ...
    # a root of the regional graph rather than vanishing.
    is_regional_edge = pl.col("parent_id").is_null() | pl.col("parent_is_regional")
    collapsed = collapse_to_lowest_qid(regional_items.filter(is_regional_edge))
    regional = promote_orphans_to_roots(regional_items, collapsed)

    output_dir.mkdir(parents=True, exist_ok=True)
    regional_path = output_dir / "7_regional_hierarchy.parquet"
